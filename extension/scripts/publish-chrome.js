...
const ZIP_PATH = process.env.CHROME_ZIP_PATH || 'extension/.output/jobfoundry-extension-chrome.zip';
...
if (!EXTENSION_ID || !PUBLISHER_ID || !CLIENT_ID || !CLIENT_SECRET || !REFRESH_TOKEN) {
  console.warn(
    '[publish-chrome] Notice: CHROME_EXTENSION_ID, CHROME_PUBLISHER_ID, CHROME_CLIENT_ID, CHROME_CLIENT_SECRET, or CHROME_REFRESH_TOKEN is not configured. Skipping Chrome Web Store publish step.'
  );
  process.exit(0);
}
