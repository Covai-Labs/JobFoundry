...
export interface JobDetailProps {
  ...
  /** Called after a successful delete (e.g. modal shell closes itself). */
  onClose?: () => void;
}
...
/**
 * Single job-detail implementation shared by every surface (split-workbench
 * pane, /jobs/:id modal, ...). All feature logic lives here exactly once —
 * add rescore/decant/sanitize/tailor/status affordances here, not in shells.
 */
export const JobDetail: React.FC<JobDetailProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'fit' | 'diff' | 'notes' | 'copilot'>('fit');
  ...
  const handleCopyLink = async () => {
    if (!job) return;
    const targetJobId = job.id;
    const url = `${window.location.origin}/jobs/${targetJobId}`;
    try {
  ...
  const missingDescription = !job.description || job.description.trim().length < 50;
    ...
    <div className="triage-workbench">
      ...
      <div className="triage-workbench-tabs">
        ...
        <button
          ...
          className={`triage-tab-btn ${activeTab === 'copilot' ? 'active' : ''}`}
        >
          <Zap size={14} /> Copilot
        </button>
      </div>
        ...
        {activeTab === 'fit' && (
          <div>
            ...
            <div>
              ...
              {!editingDesc && missingDescription && (
                <div
                  style={{
